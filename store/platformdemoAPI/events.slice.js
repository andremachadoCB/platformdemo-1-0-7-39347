...
export const api_v1_event_list = createAsyncThunk("events/api_v1_event_list", async payload => {
  const response = await apiService.api_v1_event_list(payload);
  return response.data;
});
export const api_v1_event_create = createAsyncThunk("events/api_v1_event_create", async payload => {
  const response = await apiService.api_v1_event_create(payload);
  return response.data;
});
export const api_v1_event_retrieve = createAsyncThunk("events/api_v1_event_retrieve", async payload => {
  const response = await apiService.api_v1_event_retrieve(payload);
  return response.data;
});
export const api_v1_event_update = createAsyncThunk("events/api_v1_event_update", async payload => {
  const response = await apiService.api_v1_event_update(payload);
  return response.data;
});
export const api_v1_event_partial_update = createAsyncThunk("events/api_v1_event_partial_update", async payload => {
  const response = await apiService.api_v1_event_partial_update(payload);
  return response.data;
});
export const api_v1_event_destroy = createAsyncThunk("events/api_v1_event_destroy", async payload => {
...
const eventsSlice = createSlice({
  ...
  extraReducers: {
    ...
    [api_v1_event_retrieve.pending]: (state, action) => {
      if (state.api.loading === "idle") {
        state.api.loading = "pending";
      }
    },
    [api_v1_event_retrieve.fulfilled]: (state, action) => {
      if (state.api.loading === "pending") {
        state.entities = [...state.entities.filter(record => record.id !== action.payload.id), action.payload];
        state.api.loading = "idle";
      }
    },
    [api_v1_event_retrieve.rejected]: (state, action) => {
      if (state.api.loading === "pending") {
        state.api.error = action.error;
        state.api.loading = "idle";
      }
    ...
    [api_v1_event_destroy.pending]: (state, action) => {
      if (state.api.loading === "idle") {
        state.api.loading = "pending";
      }
    },
    [api_v1_event_destroy.fulfilled]: (state, action) => {
      if (state.api.loading === "pending") {
        state.entities = state.entities.filter(record => record.id !== action.meta.arg?.id);
        state.api.loading = "idle";
      }
    },
    [api_v1_event_destroy.rejected]: (state, action) => {
      if (state.api.loading === "pending") {
        state.api.error = action.error;
        state.api.loading = "idle";
      }
